...
void analyze_hand(int num_in_suits[], int num_in_rank[])
{
	...
	if(smallest_rank == 0 && num_in_rank[12] == 1) //increase num_consec if we have a 2 AND an ace. This allows for low ace straights.
		num_consec++;
...
void print_result(void)
{
	if (straight && flush && smallest_rank == 8) printf("Royal flush");

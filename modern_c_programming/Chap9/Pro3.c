...
void generate_random_walk(char walk[10][10])
{
	...
	for (int a = 0; a < 10; a++) 			//
		for (int b = 0; b < 10; b++) 		// This part initializes the walk array
			walk[a][b] = '.';  		//
	walk[0][0]='a'; 				//
	...
	for (int a = 0; a < STEPS; a++){
		...
		int blocked[4]={0}; 			// Initializing an array to keep track of blocked sides on the walk
		
		if (locY != 0){ 				// 		   
			if (walk[locY - 1][locX] != '.') 	// If we're on the first row of the walk, the way up is blocked. 
		...
		if (locY != 9){ 				//
			if (walk[locY + 1][locX] != '.') 	// If we're on the last row of the walk, the way down is blocked.
				blocked[2]=1; 			// If we're not (on the last row of the walk), then we check to see if the location directly below ours is empty.
		...
		if (locX != 0) { 				//
			if (walk[locY][locX - 1] != '.') 	// If we're on the first column of the walk, the way left is blocked.
				blocked[3]=1; 			// If we're not (on the first column of the walk), then we check to see if the location directly left of our current position is empty.
		...
		if (locX != 9) { 				//
			if (walk[locY][locX + 1] != '.') 	// If we're on the last column of the walk, the way left is blocked.
				blocked[1]=1; 			// If we're not (on the last column of the walk), then we check to see if the location directly right of our current position is empty.
		...
		switch(move)
		{
			case 1:  						// Move 1 = Move up
				...
				walk[locY - 1][locX] = alphabets[a]; 		// Change the location above ours to a letter.
			...
			case 2: 						// Move 2 = Move right
				...
				walk[locY][locX + 1] = alphabets[a]; 		// Changing the location right to our current position to a letter.
			...
			case 3: 						// Move 3 = Move down
				...
				walk[locY + 1][locX] = alphabets[a]; 		// Changing the location below our current position to a letter
			...
			case 4: 						// Move 4 = Move left
				...
				walk[locY][locX - 1] = alphabets[a]; 		// Changing the location to the left of our current position to a letter.
